...
Both the user config (config.py) and the per-tank mode cache (mode_state.py) live
under a single AppData parent so the modpack never wipes them on reinstall:

    %APPDATA%/zanju_wot_mods_cache/research-progress-bar/{config.json, cache.json}
"""
